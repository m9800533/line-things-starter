...
function handlerToggleLock(lockState) {
...
function liffToggleDeviceLockState(state) {
    ...
    window.lockCharacteristic.writeValue(
        state ? new Uint8Array([1]) : new Uint8Array([0])
    ).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
}
function liffToggleDeviceOpenState() {
    // open: 0x10
    console.log(2);
    window.lockCharacteristic.writeValue(new Uint8Array([2])).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
}
...
function liffToggleDevicekeepOpenState() {
    // open: 0x100
    console.log(4);
    window.lockCharacteristic.writeValue(new Uint8Array([4])).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
}
